...
def classify_fn(n=0.5, m=0.5, prop=0.5):
    print('\n Process Started')
    clf_utils.timestamp()

    split = 0.25
    num_pds = 100  # number of bcc and fcc structures
    num_h = 100    # number of hcp structures
    classes = []
    ...
    if n in noise:
        ...
        print(' Please select a noise level\
              from sigma = [0.0, 0.25, 0.5, 0.75, 1.0]')
        print(' Program terminating')
        return None
    ...
    num_train = num_b + num_f + num_h
    data = clf_utils.makeDiagrams(num_b, num_f, num_h)
    data_pts = clf_utils.getData()

    print(' BCC = {}\tFCC = {}\tHCP = {}'.format(num_b, num_f, num_h))
    ...
    if num_f > 0:
        ...
        data.make_fcc_dgms()
    if num_h > 0:
        data_pts.read_data(h_train, data)
        data.make_hcp_dgms()
    data.dgm_lists()
